(() => {
  ...
  function getProfileSettings() {
    ...
    const map = {
      responsive: { stepLength: 0.72, sensitivity: 0.82, minStepGap: 180 },
      balanced: { stepLength: 0.72, sensitivity: 1.05, minStepGap: 230 },
      stable: { stepLength: 0.72, sensitivity: 1.35, minStepGap: 310 }
    };
  ...
  function drawMap(canvas, session, isReturn) {
    ...
    const height = Math.floor(Math.max(rect.height, 560) * dpr);
  ...
  function computeViewport(w, h, points) {
    ...
    const useFollow = (state.data.settings.viewMode === 'follow') || state.centerOnCurrent;
    ...
    const rangeX = Math.max(8, maxX - minX);
    const rangeY = Math.max(8, maxY - minY);
    const pad = Math.min(120, Math.max(54, w * 0.08));
    const fitScale = Math.min((w - pad * 2) / rangeX, (h - pad * 2) / rangeY, base);
    const scale = Math.max(7, fitScale);
  ...
  function drawScene(ctx, w, h, floor, origin, scale) {
    ...
    if ((state.data.settings.mapDetail || 'detailed') === 'detailed') {
      drawPerspectiveGlow(ctx, w, h);
      drawMeterGrid(ctx, w, h, origin, scale);
  ...
  function drawPerspectiveGlow(ctx, w, h) {
    const glow = ctx.createRadialGradient(w * 0.52, h * 0.45, 0, w * 0.52, h * 0.45, Math.max(w, h) * 0.62);
    glow.addColorStop(0, 'rgba(96,165,250,0.10)');
    glow.addColorStop(0.48, 'rgba(14,165,233,0.035)');
    glow.addColorStop(1, 'rgba(2,6,23,0.42)');
    ctx.fillStyle = glow;
    ctx.fillRect(0, 0, w, h);
  }
  ...
  function drawFloorPlate(ctx, w, h, floor) {
    ctx.save();
    ctx.translate(w - 180, h - 88);
    ctx.fillStyle = 'rgba(15, 23, 42, 0.58)';
    ctx.strokeStyle = 'rgba(96,165,250,0.18)';
    ctx.lineWidth = 1;
    roundRect(ctx, 0, 0, 150, 54, 18);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = '#dbeafe';
    ctx.font = '900 13px Inter, sans-serif';
    ctx.fillText('Active floor', 18, 22);
    ctx.fillStyle = '#60a5fa';
    ctx.font = '950 16px Inter, sans-serif';
    ctx.fillText(floor, 18, 42);
    ctx.restore();
  }
  ...
  function drawRouteArrows(ctx, points, origin, scale, color) {
    ...
    ctx.save();
    ctx.fillStyle = color;
    for (let i = 1; i < points.length; i += Math.max(3, Math.floor(points.length / 12))) {
  ...
  function drawCheckpointMarker(ctx, pt, name) {
    ...
    ctx.bezierCurveTo(pt.x - 16, pt.y + 2, pt.x - 13, pt.y - 18, pt.x, pt.y - 18);
    ctx.bezierCurveTo(pt.x + 13, pt.y - 18, pt.x + 16, pt.y + 2, pt.x, pt.y + 18);
    ctx.fill();
    ctx.shadowBlur = 0;
    ...
    ctx.fillText('◆', pt.x - 4, pt.y + 1);
    ctx.fillStyle = 'rgba(15,23,42,0.78)';
    ctx.strokeStyle = 'rgba(245,158,11,0.35)';
    const label = String(name || 'Checkpoint').slice(0, 24);
    ctx.font = '850 12px Inter, sans-serif';
    const labelW = Math.min(180, ctx.measureText(label).width + 22);
    roundRect(ctx, pt.x + 18, pt.y - 26, labelW, 28, 12);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = '#fef3c7';
    ctx.fillText(label, pt.x + 29, pt.y - 8);
    ctx.restore();
  }
  ...
  function drawHeading(ctx, point, origin, scale) {
    ...
    const rad = (state.heading * Math.PI) / 180;
    const end = { x: pt.x + Math.sin(rad) * 42, y: pt.y - Math.cos(rad) * 42 };
    ctx.save();
  ...
  function drawFloorBadge(ctx, w, h, floor, viewport) {
    ...
    roundRect(ctx, 16, h - 58, 190, 40, 14);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = '#cbd5e1';
    ctx.font = '850 12px Inter, sans-serif';
    ctx.fillText(`${floor} • ${viewport.fitted ? 'auto-fit' : 'follow'}`, 32, h - 34);
    ctx.restore();
  }
  ...
  function bindEvents() {
    ...
    ui.centerBtn.addEventListener('click', () => { state.panX = 0; state.panY = 0; state.centerOnCurrent = !state.centerOnCurrent; state.data.settings.viewMode = state.centerOnCurrent ? 'follow' : 'fit'; renderSettings(); drawAll(); toast(state.centerOnCurrent ? 'Following your position.' : 'Showing the full route.'); });
